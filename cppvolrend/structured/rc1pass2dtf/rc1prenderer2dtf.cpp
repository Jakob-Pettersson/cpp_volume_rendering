...
RayCasting1Pass2DTF::RayCasting1Pass2DTF ()
  : m_glsl_transfer_function(nullptr)
  , m_glsl_2d_transfer_function(nullptr)
...
void RayCasting1Pass2DTF::Clean ()
{
  if (m_glsl_transfer_function) delete m_glsl_transfer_function;
  m_glsl_transfer_function = nullptr;

  if (m_glsl_2d_transfer_function) delete m_glsl_2d_transfer_function;
  m_glsl_2d_transfer_function = nullptr;
...
void RayCasting1Pass2DTF::ReloadShaders ()
{
  cp_shader_rendering->Reload();
  m_rdr_frame_to_screen.ClearShaders();
}

bool RayCasting1Pass2DTF::Init (int swidth, int sheight)
{
  if (IsBuilt()) Clean();
  ...
  m_glsl_2d_transfer_function = m_ext_data_manager->GetCurrent2DTransferFunction()->GenerateTexture_2D_RGBt();

  // Create Rendering Buffers and Shaders
  CreateRenderingPass();
  gl::ExitOnGLError("RayCasting1Pass2DTF: Error on Preparing Models and Shaders");
...
bool RayCasting1Pass2DTF::Update (vis::Camera* camera)
{
  ...
  gl::Shader::Unbind();
  gl::ExitOnGLError("RayCasting1Pass2DTF: After Update.");
  return true;
}

void RayCasting1Pass2DTF::Redraw ()
{
...
void RayCasting1Pass2DTF::MultiSampleRedraw ()
{
  m_rdr_frame_to_screen.ClearTexture();
...
void RayCasting1Pass2DTF::DownScalingRedraw ()
{
  m_rdr_frame_to_screen.ClearTexture();
...
void RayCasting1Pass2DTF::UpScalingRedraw ()
{
  m_rdr_frame_to_screen.ClearTexture();
...
void RayCasting1Pass2DTF::SetImGuiComponents ()
{
  ImGui::Separator();
  ImGui::Text("Step Size: ");
  if (ImGui::DragFloat("###RayCasting1Pass2DTFUIIntegrationStepSize", &m_u_step_size, 0.01f, 0.01f, 100.0f, "%.2f"))
  {
...
void RayCasting1Pass2DTF::CreateRenderingPass ()
{
  ...
  cp_shader_rendering->AddShaderFile(CPPVOLREND_DIR"structured/rc1pass2dtf/ray_marching_1p_2dtf.comp");
  ...
  if (m_glsl_2d_transfer_function)
    cp_shader_rendering->SetUniformTexture2D("Tex2DTransferFunc", m_glsl_2d_transfer_function->GetTextureID(), 4);
  if (m_apply_gradient_shading && m_ext_data_manager->GetCurrentGradientTexture())
    cp_shader_rendering->SetUniformTexture3D("TexVolumeGradient", m_ext_data_manager->GetCurrentGradientTexture()->GetTextureID(), 3);
  ...
  bool use_2d_tf = *(m_ext_data_manager->GetUse2DTransferFunction());
  cp_shader_rendering->SetUniform("Use2DTransferFunction", use_2d_tf);

  cp_shader_rendering->BindUniforms();
  cp_shader_rendering->Unbind();
}

void RayCasting1Pass2DTF::DestroyRenderingPass ()
{
...
void RayCasting1Pass2DTF::RecreateRenderingPass ()
{
  DestroyRenderingPass();
  CreateRenderingPass();

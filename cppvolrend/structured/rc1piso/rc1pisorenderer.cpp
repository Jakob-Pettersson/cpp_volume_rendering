...
RayCasting1PassIso::RayCasting1PassIso()
  ...
  ,m_u_stepsize(0.5f)
  ,m_u_surfacecolor(0.7f, 0.3f, 0.3f, 1.0f)
{
}
...
bool RayCasting1PassIso::Update(vis::Camera* camera)
{
  ...
  cp_shader_rendering->SetUniform("StepSize", m_u_stepsize);
  cp_shader_rendering->SetUniform("SurfaceColor", m_u_surfacecolor);
...
void RayCasting1PassIso::SetImGuiComponents()
{
  ...
  if (ImGui::DragFloat("###RayCasting1PassIsoUIStepSize", &m_u_stepsize, 0.01f, 0.01f, 2.f, "%.2f"))
  {
    m_u_stepsize = std::max(std::min(m_u_stepsize, 2.f), 0.01f); //When entering with keyboard, ImGui does not take care of the min/max.
    SetOutdated();
  }

  ImGui::Text("Surface color: ");
  if (ImGui::ColorEdit4("Color", &m_u_surfacecolor[0])) 
  {
	SetOutdated();
  }
